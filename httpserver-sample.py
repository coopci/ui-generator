...
class SimpleHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def deal_post_data1(self):
        ...
        line = self.rfile.readline()
        remainbytes -= len(line)
        fn = re.findall(r'Content-Disposition.*name="file1"; filename="(.*)"', line)
        if not fn:
        ...
        while remainbytes > 0:
            ...
            if boundary in line:
                ...
                out.write(preline)
                out.close()
                return (True, "File '%s' upload success!" % fn.encode("utf-8"))
            else:
                out.write(preline)
                preline = line
    ...
    def deal_post_data2(self):
        ...
        try:
            ...
            pf = form['file1']
            self.save_file(pf)
            saved_fns = saved_fns + pf.filename
            # self.wfile.write(pf.value)
            return (True, "File(s) '%s' upload success!" % saved_fns)
        except IOError:
            return (False, "Can't create file to write, do you have permission to write?")

    def save_file(self, file):
        outpath = os.path.join("", file.filename.decode("utf-8"))
